...
import type { LessonPlanFormData, GeneratedPlan, Lesson, LessonActivity } from "@/pages/lesson/types/lessonPlan";
import { generateLessonPlan, regenerateLesson, generateActivity } from "@/pages/lesson/services/ragService";
...
const CreateLessonPlan = () => {
  ...
  const handleFormSubmit = useCallback(async (data: LessonPlanFormData) => {
    setFormData(data);
    setCurrentStep(2);
    ...
    try {
      ...
      setCurrentStep(3);
      toast({ title: "✅ Plano gerado!", description: `${result.lessons.length} aulas criadas com sucesso.` });
    } catch (error) {
      toast({ title: "Erro ao gerar plano", description: error instanceof Error ? error.message : "Erro desconhecido", variant: "destructive" });
      setCurrentStep(1);
    } finally {
  ...
  const handleDeleteLesson = useCallback((index: number) => {
    setLessons((prev) => prev.map((l, i) => (i === index ? { ...l, isDeleted: true } : l)));
    toast({ title: "Aula removida", description: "A aula foi marcada para exclusão." });
  }, [toast]);
  ...
  const handleRegenerate = useCallback(async (index: number) => {
    ...
    try {
      ...
      setLessons((prev) => prev.map((l, i) => (i === index ? { ...newLesson, isDeleted: false } : l)));
      toast({ title: "Aula regenerada!", description: `Aula ${newLesson.lesson_order} atualizada.` });
    } catch (error) {
      toast({ title: "Erro ao regenerar", description: error instanceof Error ? error.message : "Erro desconhecido", variant: "destructive" });
    } finally {
      setRegeneratingIndex(null);
    }
  }, [formData, lessons, toast]);

  const handleGenerateActivity = useCallback(async (index: number) => {
    ...
    try {
      ...
      setLessons((prev) => prev.map((l, i) => (i === index ? { ...l, activity } : l)));
      toast({ title: "Atividade gerada!", description: `Atividade criada para a Aula ${lesson.lesson_order}.` });
    } catch (error) {
      toast({ title: "Erro ao gerar atividade", description: error instanceof Error ? error.message : "Erro desconhecido", variant: "destructive" });
    } finally {
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      toast({ title: "🎉 Plano salvo!", description: "O plano de aulas foi salvo com sucesso no PocketBase." });
    } catch (error) {
        ...
        <div className="container max-w-4xl py-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-primary-foreground/70">Criação assistida por IA</p>
            </div>
          </div>
        </div>
      </header>
      ...
      <main className="container max-w-4xl pb-32">
          ...
          <div className="space-y-5 animate-fade-in">
            ...
            {plan && (
              <div className="glass-card rounded-lg p-5 space-y-3">
                <h2 className="font-bold text-foreground text-lg">📋 Resumo do Plano</h2>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                  <div>
                    <span className="text-xs font-semibold text-muted-foreground uppercase">Ementa</span>
                    <p className="text-foreground mt-0.5">{plan.syllabus}</p>
                  </div>
                  <div>
                    <span className="text-xs font-semibold text-muted-foreground uppercase">Objetivo Geral</span>
                    <p className="text-foreground mt-0.5">{plan.general_objective}</p>
                  </div>
                  <div>
                    <span className="text-xs font-semibold text-muted-foreground uppercase">Metodologia</span>
                    <p className="text-foreground mt-0.5">{plan.methodology}</p>
                  </div>
                  <div>
                    <span className="text-xs font-semibold text-muted-foreground uppercase">Avaliação</span>
        ...
        {currentStep === 4 && isSaving && (
          <div className="flex flex-col items-center justify-center py-20 animate-fade-in">
            ...
            <h3 className="text-xl font-bold text-foreground mt-6">Salvando no PocketBase...</h3>
          </div>
        )}
        ...
        {currentStep === 4 && !isSaving && (
          <div className="flex flex-col items-center justify-center py-20 animate-fade-in">
            ...
            <h3 className="text-2xl font-bold text-foreground">Plano Salvo com Sucesso!</h3>
            <p className="text-muted-foreground mt-2">Todas as aulas foram salvas no PocketBase.</p>
            <Button className="mt-6 gradient-primary text-primary-foreground" onClick={() => { setCurrentStep(1); setPlan(null); setLessons([]); setFormData(null); }}>
              Criar Novo Plano
            </Button>
          </div>
        )}
